...
_DIGEST_FRAMING = (
    ...
    "- ATTRIBUTE INVARIANTS (constraints on a single entity's attributes) and ROLE-ASSIGNMENT "
    "CONSTRAINTS (which roles a user may not hold at once): CONTEXT ONLY — they inform your reasoning "
    "but never by themselves add a grant or a prohibition for a (focal, candidate) pair.\n"
...
_DENY_RULES = (
    ...
    "Read this from BOTH the scenario policy AND the focal/candidate DESCRIPTIONS, symmetrically with "
    "grants: a prohibition stated only in a description (e.g. 'does not manage the issue tracker', "
    "'works … not in source') is a valid DENY trigger — NOT a mere attribute invariant to treat as "
    "context. The grants-only baseline is NEVER a source of prohibitions. Silence about a pair "
    "imposes NOTHING on anything else — it never denies. A digested policy states exclusivity as "
    "explicit per-pair denies of exactly this kind, so there is no 'only …' wording to interpret and "
    "no complement to derive.\n"
