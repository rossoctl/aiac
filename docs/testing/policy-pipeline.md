...
It imports two shared modules: `test/system/scenario_uc1.py` — the canonical `github-agent`
...
A `@pytest.mark.system` test that drives the **whole identity→policy→enforcement pipeline** —
...
- **LLM nondeterminism, contained.** The in-cluster PRB LLM is pinned to `temperature=0`, and the
  ...
  remains, which is why the suite is `@pytest.mark.system`, out of default CI.
...
> [../components/aiac-agent/policy-rules-builder.md](../specs/components/aiac-agent/policy-rules-builder.md),
