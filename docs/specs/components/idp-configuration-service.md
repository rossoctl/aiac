...
| GET | `/services/{service_id}/roles` | `admin.get_client_roles(service_id)` | **Client roles defined on this service's client** — an agent's own roles (`R_A`). See "Agent roles are client roles" below. |
