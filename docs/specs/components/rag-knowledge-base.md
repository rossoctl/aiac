...
## Deployment
Kubernetes **StatefulSet** in the RAG Pod, co-located with the RAG Ingest Service and Policy Guardrails Agent containers. Exposed via the `aiac-rag-service` ClusterIP Service on port 8000 (ChromaDB default). Manifest: `rag-statefulset.yaml`.
...
| Policy Guardrails Agent | Read (evaluation context, e.g. the pre-update version of a `doc_id`) | Either collection, matching the document under verification |
