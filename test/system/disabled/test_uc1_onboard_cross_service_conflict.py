...
real PRB LLM in-pod, so it is marked ``system``:

    .venv/bin/pytest test/system/test_uc1_onboard_cross_service_conflict.py -m system -v
...
# ``system`` — needs a live cluster; the hermetic default run deselects it. Phase 2 drives the real
# PRB LLM in-pod (the cluster's own endpoint), so no separate ``llm`` tag.
pytestmark = [
    pytest.mark.system,
