...
class TestService:
    ...
    def test_type_tool(self):
        s = Service.model_validate({"id": "c1", "clientId": "c1", "name": "tool-svc", "enabled": True, "type": "Tool"})
        assert s.type == "Tool"

    def test_type_none_when_absent(self):
        s = Service.model_validate({"id": "c2", "clientId": "c2", "name": "bare", "enabled": True})
        assert s.type is None

    def test_optional_fields_absent(self):
        s = Service.model_validate({"id": "c2", "serviceId": "c2", "enabled": False})
        assert s.name is None
        assert s.description is None
        assert s.type is None

    def test_description_present(self):
        s = Service.model_validate({"id": "c1", "clientId": "c1", "enabled": True, "description": "a desc"})
        assert s.description == "a desc"

    def test_description_absent_is_none(self):
        s = Service.model_validate({"id": "c1", "clientId": "c1", "enabled": True})
        assert s.description is None
    ...
    def test_serviceId_provided_directly_when_clientId_absent(self):
        s = Service.model_validate({"id": "c1", "enabled": True, "serviceId": "explicit-svc"})
...
class TestHashAndEquality:
    def _service(self, id_: str) -> Service:
        return Service.model_validate({"id": id_, "clientId": id_, "enabled": True})

    def _role(self, id_: str) -> Role:
        return Role.model_validate({"id": id_, "name": "r", "composite": False})

    def _scope(self, id_: str) -> Scope:
        return Scope.model_validate({"id": id_, "name": "s"})

    # Service

    def test_service_equal_ids_are_equal(self):
        assert self._service("x") == self._service("x")

    def test_service_equal_ids_same_hash(self):
        assert hash(self._service("x")) == hash(self._service("x"))

    def test_service_different_ids_not_equal(self):
        assert self._service("a") != self._service("b")

    def test_service_usable_as_dict_key(self):
        svc = self._service("svc1")
        d = {svc: "val"}
        assert d[svc] == "val"

    # Role

    def test_role_equal_ids_are_equal(self):
        assert self._role("x") == self._role("x")

    def test_role_equal_ids_same_hash(self):
        assert hash(self._role("x")) == hash(self._role("x"))

    def test_role_different_ids_not_equal(self):
        assert self._role("a") != self._role("b")
    ...
    def test_scope_equal_ids_are_equal(self):
        assert self._scope("x") == self._scope("x")

    def test_scope_equal_ids_same_hash(self):
        assert hash(self._scope("x")) == hash(self._scope("x"))

    def test_scope_different_ids_not_equal(self):
        assert self._scope("a") != self._scope("b")

    def test_scope_usable_as_dict_key(self):
        scope = self._scope("sc1")
        d = {scope: "val"}
        assert d[scope] == "val"

    # Cross-type: same id, different type

    def test_service_not_equal_to_role_with_same_id(self):
        assert self._service("x") != self._role("x")

    def test_service_not_equal_to_scope_with_same_id(self):
        assert self._service("x") != self._scope("x")

    def test_role_not_equal_to_scope_with_same_id(self):
        assert self._role("x") != self._scope("x")
