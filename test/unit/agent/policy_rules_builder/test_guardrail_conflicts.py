...
docs/evaluation/policy-eval-scenarios.md for the broader guardrail-scenario catalog
this test belongs to.

Requires a live LLM (``@pytest.mark.llm``); skips when ``LLM_BASE_URL`` is unset. It does
...
@pytest.mark.xfail(
    strict=True,
    reason="no whole-document guardrail exists yet — see docs/evaluation/policy-eval-scenarios.md",
)
