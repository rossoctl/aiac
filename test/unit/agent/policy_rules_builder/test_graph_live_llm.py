...
Gating: the module is marked ``llm`` — it calls a real external LLM endpoint but needs no
cluster. The hermetic default run deselects ``llm`` (its collected count is unchanged by
this suite), and it is selected on its own (``-m llm``) without a cluster or Keycloak,
unlike the cluster-bound ``system`` suite. Every test also first calls
...
from test.system import scenario_uc1 as scn
